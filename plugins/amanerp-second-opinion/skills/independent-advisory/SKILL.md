...
- A same-model structured-output retry is also explicit: before requesting authorization, disclose the selected profile's exact aggregate ledger (critical: USD 10 total; deep/standard advisory: USD 5 total). Attempt one may request the full aggregate ceiling. After an eligible failure, attempt two requests the verified remaining balance—aggregate ceiling minus attempt-one reported cost, rounded down to cents—and is preempted when less than USD 0.10 remains. Use `--structured-output-attempts 2 --acknowledge-retry-cost` only after the user authorizes another attempt following that disclosure. A generic rerun permission given without the cost disclosure is insufficient. The runner retries only Claude's structured-output retry-exhausted outcome and never changes model, effort, input, prompt, schema, or isolation controls.
- The second attempt replays identical decision content and is useful only for a transient/non-deterministic failure. The remaining-budget ledger prevents retry authorization from reducing the first attempt's ceiling but does not guarantee enough balance for a second attempt.
