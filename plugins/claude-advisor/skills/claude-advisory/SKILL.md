...
description: Ask the locally authenticated Claude Code CLI for an explicit, bounded second opinion on a consequential engineering or product decision. Use only when the user explicitly requests Claude, this skill, another model's opinion, or independent Claude recommendations; never invoke it implicitly.
---

# Claude Advisory

Use this skill to add an independent Claude perspective, not to replace repository evidence or make a decision by authority.

## Safety contract

- Invocation must be explicit. Do not infer permission from words such as “critical,” “thorough,” or “second opinion” unless Claude or this skill is named.
...
Output defaults to `.codex/claude-advisor/runs/` in the current workspace. The runner prints a compact JSON summary with the exact artifact paths.
